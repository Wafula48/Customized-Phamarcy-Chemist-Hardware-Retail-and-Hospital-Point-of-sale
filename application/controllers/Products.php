...
class Products extends CI_Controller
{
 ...
 $data['total_stock_value'] = $this->Products_model->get_total_stock_value();
 	$data['total_salesprofit'] = $this->Products_model->get_total_salesprofit();
    ...
    public function addproduct()
    {
        ...
        $data['suppliers'] = $this->Constant_model->getDataOneColumn('suppliers', 'status', '1');
    ...
    public function updateProduct()
    {
        ...
        if (empty($code)) {
         ...
         // Add supplier_id to the update data array:
$upd_data = array(
    'name' => $name,
    'category' => $category,
    'supplier_id' => $this->input->post('supplier'), // Add this line
...
public function insertProduct()
{
    $code = strip_tags($this->input->post('code'));
    $name = strip_tags($this->input->post('name'));
    $pres = strip_tags($this->input->post('pres'));
    $category = strip_tags($this->input->post('category'));
    $purchase = strip_tags($this->input->post('purchase'));
    $retail = strip_tags($this->input->post('retail'));
    $expiry_date = strip_tags($this->input->post('expiry_date'));
    $us_id = $this->session->userdata('user_id');
    $tm = date('Y-m-d H:i:s', time());

    // Validation
    if (empty($code) || empty($name) || empty($pres) || empty($category) || empty($purchase) || empty($retail) || empty($expiry_date)) {
        $this->session->set_flashdata('alert_msg', array('failure', 'Add New Product', 'All fields are required!'));
    ...
    if (count($ckPcodeData) > 0) {
        $this->session->set_flashdata('alert_msg', array('failure', 'Add New Product', "Product code '$code' already exists."));
        redirect(base_url() . 'products/addproduct');
    }
    ...
    $ins_data = array(
        ...
        'thumbnail' => 'no_image.jpg', // Default image
        'created_user_id' => $us_id,
        'created_datetime' => $tm,
        'status' => '1',
    );
    $product_id = $this->Constant_model->insertDataReturnLastId('products', $ins_data);
    ...
    if (!empty($_FILES['uploadFile']['name'])) {
        ...
        foreach ($thumbnail_sizes as $size => $dimensions) {
            $size_path = $upload_path . $size . '/' . $code . '/';
            if (!is_dir($size_path)) {
        ...
        if ($this->upload->do_upload('uploadFile')) {
            ...
            foreach ($thumbnail_sizes as $size => $dimensions) {
                $config = array(
                    'image_library' => 'gd2',
                    'source_image' => $upload_path . $new_file_name,
                    'new_image' => $upload_path . $size . '/' . $code . '/' . $new_file_name,
                    'maintain_ratio' => true,
            ...
            $upd_data = array('thumbnail' => $new_file_name);
            $this->Constant_model->updateData('products', $upd_data, $product_id);

            // Remove original uploaded file (we only keep thumbnails)
            unlink($upload_path . $new_file_name);
        } else {
            $error = $this->upload->display_errors();
            log_message('error', 'Upload failed: ' . $error);
            $this->session->set_flashdata('alert_msg', array('warning', 'Add New Product', 'Product added but image upload failed: ' . $error));
            redirect(base_url() . 'products/addproduct');
        }
    }

    $this->session->set_flashdata('alert_msg', array('success', 'Add New Product', "Product added successfully!"));
    redirect(base_url() . 'products/addproduct');
}
    // Insert New Product;
    /*public function insertProduct()
