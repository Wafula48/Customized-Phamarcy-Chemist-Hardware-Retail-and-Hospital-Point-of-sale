...
class Inventory extends CI_Controller
{
    ...
    public function index()
    {
        $this->load->view('dashboard', 'refresh');
		 $data['low_stock'] = $this->Inventory_model->get_low_stock_products(); // Fetch products with less than 10 quantity
        $data['expiry_stock'] = $this->Inventory_model->get_expiry_stock_products();
		$data['results'] = $this->Inventory_model->fetch_product_data(10, 0);  // Fetch all inventory data (pagination example)
    ...
    public function view()
    {
        ...
        $data['best_sellers'] = $this->getBestSellingProducts(10);
        $this->pagination->initialize($config);

        $page = ($this->uri->segment(3)) ? $this->uri->segment(3) : 0;
        $data['results'] = $this->Inventory_model->fetch_product_data_with_supplier($config['per_page'], $page);
   ...
   public function low_stock()
    {
        $data['low_stock_details'] = $this->Inventory_model->get_low_stock_products(); // Fetch low stock product details
        $this->load->view('low_stock_view', $data); // Render a separate page for low stock details
    }
	  public function expiry_stock()
    {
        $data['expiry_stock_details'] = $this->Inventory_model->get_expiry_stock_products(); // Fetch low stock product details
        $this->load->view('expiry_stock_view', $data); // Render a separate page for low stock details
    }
...
public function fetch_product_data_with_supplier($limit, $start) {
    ...
    return $result;
	  ...
	  public function bestseling()
    {
        $data['best_sellers'] = $this->getBestSellingProducts(10); // Fetch low stock product details
        $this->load->view('bestseling', $data); // Render a separate page for low stock details
    }
...
public function export_expiry_stock_products() {
    $five_months_from_now = date('Y-m-d', strtotime('+5 months'));

    // Fetch products expiring within 5 months
    $this->db->select('products.name, products.expiry_date');
    $this->db->from('products');
    $this->db->where('products.expiry_date <=', $five_months_from_now);
    $query = $this->db->get();
    $products = $query->result();

    // Set headers for Excel download
    header("Content-Type: application/vnd.ms-excel");
    header("Content-Disposition: attachment; filename=expiring_products_" . date('Y-m-d') . ".xls");
    ...
    // Column Headers
    fputcsv($output, ["Product Name", "Expiry Date"], "\t");

    // Write data to Excel
    foreach ($products as $product) {
        fputcsv($output, [$product->name, $product->expiry_date], "\t");
    }
...
// Ensure this function is defined properly before using it
function getBestSellingProducts($limit = 10) {
    ...
    // SQL Query: Ensure correct table/column names
    $query = "SELECT p.name, SUM(s.total_items) AS total_sold 
              FROM orders s 
   ...
   // echo $query; // Debugging: See the final query
