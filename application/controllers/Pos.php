	...
	public function insertSale()
{
    ...
    $enable_backdate = $this->input->post('enable_backdate');
    
    if($enable_backdate && !empty($sale_date)) {
        // Validate date format
        if(!preg_match('/^\d{4}-\d{2}-\d{2}$/', $sale_date)) {
            $this->session->set_flashdata('alert_msg', array('failure', 'Invalid Date', 'Please select a valid date format (YYYY-MM-DD).'));
            redirect(base_url().'pos');
            return;
        }
        ...
        if(strtotime($sale_date) > time()) {
            $this->session->set_flashdata('alert_msg', array('failure', 'Invalid Date', 'You cannot select a future date.'));
            redirect(base_url().'pos');
            return;
        }
        
        // Permission control - only admin and manager can backdate
        $user_role = $this->session->userdata('user_role');
        if($user_role != '1' && $user_role != '2') {
            $this->session->set_flashdata('alert_msg', array('failure', 'Permission Denied', 'You are not authorized to backdate sales.'));
            redirect(base_url().'pos');
            return;
        }
        
        // Combine with current time
        $tm = $sale_date . ' ' . date('H:i:s');
    } else {
        $tm = date('Y-m-d H:i:s');
    }
    ...
    if (isset($_POST['hold_bill_submit'])) {
        ...
        // Audit logging for backdated sales
        if($enable_backdate && !empty($sale_date)) {
            $log_data = array(
                'user_id' => $user_id,
                'action' => 'Backdated Sale',
                'details' => 'Created sale with date: '.$tm.' | Order ID: '.$order_id,
                'ip_address' => $this->input->ip_address(),
                'created_datetime' => date('Y-m-d H:i:s')
            );
            $this->Constant_model->insertData('audit_logs', $log_data);
        }
    ...
    public function getProductDetail()
    {
		...
		$prod_price1 = $pcodeDtaData[0]->retail_price;
		$prod_price2 = $pcodeDtaData[0]->retail_price;
...
public function updateCartItemPrice() {
    ...
    if (empty($pcode) || !is_numeric($new_price) || $new_price <= 0) {
    ...
    foreach ($cart as &$item) {
        if ($item['row_id'] == $row_id && $item['pcode'] == $pcode) {
