...
<?php
    $total_sub_amt = 0;
        ...
        $expenseResult = $this->db->query("SELECT * FROM expenses WHERE date >= '$start_date' AND date <= '$end_date'");
        $expenseRows = $expenseResult->num_rows();

        if ($orderRows > 0) {
            $orderData = $orderResult->result();
			$expenseData = $expenseResult->result();
...
<?php
    if ($orderRows > 0) {
		...
		<div class="row" style="padding-top: 10px; padding-bottom: 10px; font-size: 18px; letter-spacing: 0.5px;">
			...
			<div class="col-md-10" style="font-weight: bold;">: <?php echo number_format($total_expenses, 2); ?></div>
		</div>
