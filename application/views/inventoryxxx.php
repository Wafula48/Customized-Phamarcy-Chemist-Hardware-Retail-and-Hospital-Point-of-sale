<?php
    ...
    foreach ($getAllInvResult->result() as $invData) {
        ...
        $getPriceResult = $this->db->query("SELECT purchase_price, retail_price FROM products WHERE code = '$each_row_code'");
        $row = $getPriceResult->row();

        $each_cost = $row->purchase_price ?? 0;
        $each_selling = $row->retail_price ?? 0;

        $total_cost_amt += ($each_row_qty * $each_cost);
        $total_sales_amt += ($each_row_qty * $each_selling);
        $total_profit_amt += ($each_row_qty * ($each_selling - $each_cost));
    }
?>
    ...
    <div class="row">
        <div class="col-lg-12 d-flex justify-content-between align-items-center">
            ...
            ">
                ...
                <div style="border-left: 2px solid white; height: 40px; margin-right: 20px;"></div>
                <div>
                    <h5 style="margin: 0;">Total Expected Sales (<?php echo $site_currency; ?>):</h5>
                    <h3 style="margin: 0;"><?php echo number_format($total_sales_amt, 2, '.', ','); ?></h3>
                </div>
                <div style="border-left: 2px solid white; height: 40px; margin-right: 20px;"></div>
                <div>
                    <h5 style="margin: 0;">Total Expected Profit (<?php echo $site_currency; ?>):</h5>
                    <h3 style="margin: 0;"><?php echo number_format($total_profit_amt, 2, '.', ','); ?></h3>
                </div>
            </div>
        </div>
    </div>
